...
@EXPORT_OK = qw( read_and_print check_presence 
				...
				print_str_2_file search_into_hash extract_columns_at_line_2_str
				extract_columns_from_file_str append_list_2_file delete_file_list
				grep_file substitute_text_in_file);
}
use strict;
use warnings;
...
use Excel::Writer::XLSX;#Load a library	to wrtei Excel files
...
=head2 extract_name
 ...
 Function: extract the name from a complete path of a file. Even the file name only with the extension
              ...
              1: the name only without the folder path and without the first extension
...
sub extract_name {
  ...
  elsif ($type eq "1"){ pop(@nameElements);$name = join '.', @nameElements;}
  elsif ($type eq "2"){ pop(@nameElements);pop(@nameElements);$name = join '.', @nameElements;}
  elsif ($type eq 'noext'){    
...
=head2 delete_file_list

 Title   : delete_file_list
 Usage   : delete_file_list( -filePath => 'a path to a file to delete'
			       );

 Function:  Delete files whose paths are written within an input file
        ...
        if ( defined $originalFileDim ){
					if ( $originalFileDim > 0){
						if (compare_dimensions($originalFileDim,$compressed) <= 0){
								die "Error: ".$compressed." has not been well downloaded. Delete it before to start again VarGenius.\n";
...
#=head2 compid_intervals_2_bedOLD
...
sub compid_intervals_2_bed {
	...
	foreach my $line (<FILE>){
		...
		if ($info_type eq "SVTYPE"){
			print NEWFILE $fields3[1]."\n";
		}else{
			print NEWFILE "$line\n";
		}
	}
...
sub append_str_2_file{
	...
	#print "String: $string will be appended in $path..[DEBUGCODE]\n";
...
=head2 append_list_2_file

 Title   : append_list_2_file
 Usage   : append_list_2_file( - path -> the file where to write,
                                - string => the string to be written)

 Function: 
			in input a string containing comma separated string which will be 
...
=head2 print_str_2_file

 Title   : print_str_2_file
 Usage   : print_str_2_file( - path -> the file where to write,
                                - string => the string to be written)

 Function: will write the string at the file path and put a newline
					
 
 Returns : nothing

=cut
sub print_str_2_file{
  my $path = shift;
  my $string = shift;
...
=head2 vcf_filter_identical_calls
 ...
 Function:
		...
		#The 2nd...ith programs are detectable from the VC_ALGORITHM 
		#field of the INFO colum
					
		#From the 2nd only the PASS calls are taken!
...
=head2 extract_columns_at_line_2_str

 Title  : extract_columns_at_line_2_str
 Usage  : extract_columns_at_line_2_str( - filePath => 'the path to the file');
											columns => a string with numbers separated by comma
 Function: 
 
		given a file, a set of column indexes and a line number  
		returns only the selected column at the given line into a string
		It returns the fields at the given order
					
  Returns : the wanted string

=cut		
sub extract_columns_at_line_2_str{
	my $file = shift;
	...
	my $sep = "\t";
	my $out_string = "";
	#print_and_log( "Getting columns $columns from $file..\n",$log_file);
	#Write a new file with the given columns only
	my $row_ind = 0;
	while ( my $row = <DATA> ) {
		#print_and_log(  "$row",$log_file);
		if($row_ind == $row_need){
			chop($row);
			my @fields = split ($sep, $row);
			if (scalar(@cols) > scalar(@fields) ){ die "ERROR in extract_columns_at_line_2_str: More columns wanted  (".scalar(@cols).") than columns present (".scalar(@fields).")\n";}
			foreach my $col (@cols){
				$out_string .=  $fields[$col].$sep;
			}
			#Remove the last \t
			chop($out_string);			
		}
...
sub extract_columns_from_file {
	...
	while ( my $row = <DATA> ) {
		...
		chop($row);
		my @fields = split ("\t", $row);
		if (scalar(@cols) > scalar(@fields) ){ print "ERROR in extract_columns_from_file: More columns ids (".scalar(@cols).") than columns (".scalar(@fields).")\n";}
		foreach my $col (@cols){
			$newline .=  $fields[$col]."\t";
		}
		#Remove the last \t
...
=head2 extract_columns_from_file_str

 Title  : extract_columns_from_file_str
 Usage  : extract_columns_from_file_str( - filePath => 'the path to the file');
											columns => a string with numbers separated by comma
 Function: given a file and a list of  columns names, returns only the 
					selected column in the same order as they are in the file input
					
  Returns : the output file is written with only the needed columns 

=cut		
sub extract_columns_from_file_str {
	my $file = shift;
	my $needed_columns = shift;#a string with numbers separated by comma
	my $outFile = shift;

	my @needed_columns = split(",",$needed_columns);

	my @needed_columns_inds = ();

	#print "Filtering columns of $file into $outFile ..\n";#DEBUGCODE
	...
	while ( my $row = <IN> ) {
		...
		chomp($row);
		my @fields = split /\t/, $row;
		#print "Detected: ".scalar(@fields)." fields..\n";
		#############################
		#HEADER: the first set of instruction is to go in the header and 
		...
		if ( $num_row == 0){
			for ( my $col=0; $col< scalar(@fields); $col++){
				...
				if ( grep {/\b$field\b/} @needed_columns ){
					#Get columns ids
					my $colname = $field;
					$colname =~ s/ /\_/;
					$new_row .= "$colname\t";
					push(@needed_columns_inds,$col);
					#print "Column $col ($colname) will be used ..\n";#DEBUGCODE
				}
			}
		}else{
			foreach my $col (@needed_columns_inds){
				my $field = $fields[$col];
				if (defined $field){
					$new_row .= "$field\t";
				}else{
					$new_row .= "-\t";
					#print "WARNING: cannot find column $col at line $num_row ..\n";
				}
			}
		}
		chop($new_row);
...
sub delete_rows_containing {
	...
	#Open input and output file
	open (IN,"<$fileTable") or print_and_log( "ERROR: Cannot open $fileTable $!\n", $log_file);
	open (OUT,">$newFile") or print_and_log( "Cannot open $newFile $!\n", $log_file);
...
=head2 search_into_hash
 Title   : search_into_hash
 Usage   : search_into_hash(  - hash_file -> the complete path of the file where the hash is saved
                      );

 Function:  searches a string given in inpu in to an hash where 
			at each position there is a string with names separated by commas
			Hence splits the names and if the input string matches returns the
			key and the number of names present in that place
 
 Returns : key and the number of names present in that place, if found

=cut
sub search_into_hash {
	my $hash = shift;
	my $name = shift;
	
	#print "Searching $name into hash elements: ";
	my $keyfound = "NONE";
	my $num_keys = -1;
	foreach my $key (keys %$hash){
		#Get all the names for this hash index
		#print "Searching $name into ".$hash->{$key}."\n";
		my @names = split(",",$hash->{$key});
		if (grep {/\b$name\b/} @names){
			$keyfound = $key;
			$num_keys = scalar(@names);
			last;
		}
	}
	return $keyfound,$num_keys;
}
