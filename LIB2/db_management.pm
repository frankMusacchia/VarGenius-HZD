...
sub update_analysis_status_locked{
	...
	if (defined $database){
		...
		eval{
			...
			#Extracts information from database defined.
			#fetch informations from the database
			$query = "UPDATE $table SET $step=1 WHERE $field1=$value1";
...
Downloaded data from https://ftp.ncbi.nlm.nih.gov/refseq/H_sapiens/RefSeqGene/
 gene_RefSeqGene
   
   This is a subset of LRG_RefSeqGene, limited to the RefSeqGene accessions for a gene.
    
        The columns are:
          NCBI taxonomy id (all 9606)
          GeneID 
          Symbol of the gene (official from HGNC when available)
          Accession.version of the RefSeqGene
  ...
  if (not (-e $omim2genes_f) ){
		...
		#Read only the rows with the 'gene' indication as MIM entry type
			if ( defined $fields[$gene_ind] ){
				if (  $fields[$gene_ind] ne '') {
					...
					#Remove all chars from description which can corrupt the query
					$omim_phenotype =~ s/[^A-Za-z0-9\-\_\s]//g;
					$omim_gene_desc =~ s/[^A-Za-z0-9\-\_\s]//g;
	...
	foreach my $gene (keys %{$genes_hash} ){
		...
		#If the gene is not present insert it..	
			...
			print_and_log("$gene newly inserted. Inserting $gene fields = $fields and values= $values...\n",$log_file);#DEBUGCODE
