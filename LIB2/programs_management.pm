...
BEGIN
{
	...
	@EXPORT_OK = qw( initialize_folders checkConfigVariables checkVariable
									 ...
									 run_FREEBAYES_vc_parallel JOB_get_state PBS_get_maximum_queue_resources
									 PBS_get_queues_info);
}
...
sub get_samples_id_from_analysis{
	...
	my $samples_ids_h = do_query_select_all($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$query,$cfg_hash->{'db_sample_id'});

	foreach my $sampleid (keys %{$samples_ids_h}){
		push(@$samples_id,$sampleid );
	}
}
...
sub save_analyses{
	...
	foreach my $analysisid (@ans_to_store){
		...
		#202004: I have commented the constrain that the flag in the database must be stored=0
		#to save the analysis because is difficult that the user remembers to use force_run 
		#while is safe that if the analysis is repeated the storage will be updated.
		#If the analysis has not been already stored or the user wants to force the run
			...
			if ( correct_type($analysisid,"positiveint") ){					
				...
				foreach my $path ( @paths ){
					my $desc = $descs[$i];
					#print_and_log("Checking if $desc matches one among IGV, SAVE, WEB.[DEBUGCODE]\n",$log_file); 
					#The description must have one among the following keywords
					if ( $desc =~ /WEB/ or $desc =~ /IGV/ or $desc =~ /SAVE/ ){
						#print_and_log("Copying $path to  $analysis_fold_st..[DEBUGCODE]\n",$log_file);
						if ( -d $path ){
					...
					#else{print_and_log(" $path will not be copied to $analysis_fold_st..\n",$log_file);}
					$i++;
				}
				##################################################
...
sub show_samplesids_from_samplesnames{
	...
	foreach my $sample_name (@samples_names){
		my $query = "SELECT ".$cfg_hash->{'db_sample_id'}." FROM  ".$cfg_hash->{'db_sample_table'}." WHERE ".$cfg_hash->{'db_sample_name'}."='$sample_name';";	
		print_and_log( "Executing: $query\n",$log_file);#DEBUGCODE
		...
		foreach my $sample_id (keys %{$res_group_sam}){
			if ( $sample_id >= 0){
				#Obtain the analysis id from the database given the sample  id
				my $analysis_id = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
									$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'db_analysis_id'},
									$cfg_hash->{'db_sample_id'},$sample_id);
									
				#Obtain the analysis id and name from the database given the sample id
				my $analysis_name = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
								$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_name'},
								$cfg_hash->{'db_analysis_id'},$analysis_id);	
				print "$analysis_name ($analysis_id) - $sample_name : $sample_id\n";
			}else{
...
=head2 JOB_is_active_queue
 ...
 Function: goes recursively through a job queue and checks if the 
...
sub JOB_is_active_queue{
	...
	my $jobdeps = JOB_get_dependencies($jobid,$log_file);
	my $jobstate = JOB_get_state($jobid,$log_file);
	
	#If length of jobdeps is 0 means that there are no dependencies or an error or is finished
	#If there are dependencies go ahed and check them
	if ( length($jobdeps) > 0){
		#print_and_log("$jobid dependencies are: $jobdeps. ",$log_file);#DEBUGCODE
		#Separate the type of waiting
		my @waits =  split(",",$jobdeps);
		foreach my $wait ( @waits){
			#print_and_log("Checking now: [$wait]: ",$log_file);#DEBUGCODE
			#Separate the elements using :
			my @elems = split(":",$wait);
			my $wtype = shift(@elems);
			#print_and_log("wait type: $wtype... ",$log_file);#DEBUGCODE
			...
			if ( $wtype eq 'afterok'){
				...
				foreach my $job2wait (@elems){
					...
					#Job id will be in the format 207165.node001@node001. We need to remove node info
					my @job2wait_pcs = split(/\./,$job2wait);
					#print_and_log("$job2wait is 'afterok' checking if ".$job2wait_pcs[0]." is active...",$log_file);#DEBUGCODE
					if ( JOB_is_active_queue($job2wait_pcs[0],$log_file) == 1){
						$retval = 1;
					}
				} 
			}
		...
		#A the end of the analysis of this job, get 1 if the job is running
		if ($jobstate eq 'R' or $jobstate eq 'Q'){
			$retval = 1;
		}
	}
		...
		foreach my $wait ( @waits){
			...
			if ( $wtype eq 'afterok'){
				...
				foreach my $job2wait (@elems){
					...
					my @job2wait_pcs = split(/\./,$job2wait);
					#print_and_log("$job2wait is 'afterok' checking if ".$job2wait_pcs[0]." is active...",$log_file);#DEBUGCODE
					if ( JOB_is_active_queue($job2wait_pcs[0],$log_file) == 0){
						$retval = 0;
					}
				} 
			}
...
sub JOB_get_dependencies{
	...
	my $qstat_out = "qstat_$job_id\_$time.txt";	
	my $command = "qstat -f $job_id > $qstat_out";
	#print_and_log( "(Executing: $command...) ",$log_file);#DEBUGCODE
	try_exec_command($command) or die "Unable to execute command: $command\n";
	
	#Dependencies string to output
	my $deps = "";
	 #If qstat output was generated
	if ( -e $qstat_out and !(-z $qstat_out) ){	
		open (QSTAT_OUT,"<$qstat_out") or die "ERROR: Cannot open $qstat_out. The program will exit..\n";
		my $start = 0;

		while (my $line = <QSTAT_OUT>){
			chop($line);
			#If the depend was found, check if continues on a newline
			if ($start == 1){
				#if ( defined $log_file){print_and_log( "reading if there is another line of depends in [$line]...",$log_file);}#DEBUGCODE
				#print $line."\n";
				#If on the next line there is not a new parameter, read line
				if( $line !~ /=/ ){
					#Remove the TAB
					$line =~ s/\s//g;
					#Add this dependencies
					$deps .= $line;
					#if ( defined $log_file){#DEBUGCODE
						#print_and_log( "adding:'$line'. Obtaning:'$deps'",$log_file);#DEBUGCODE
					#}#DEBUGCODE
				}else{
					#If dependencies are finished, set start to 0
					$start = 0;
				}
			}
			#Find the line with depend = 
			if( $line =~ /depend =/ ){
				#if ( defined $log_file){print_and_log( "depends are found...",$log_file);}	#DEBUGCODE
				#print $line."\n";
				my @fields = split("=",$line);
				my $stdeps = $fields[1];
				$stdeps =~ s/ //g;
		...
		close(QSTAT_OUT);
		#Remove the remporary qstat output
...
=head2 JOB_get_state
 ...
 Function:  uses qstat -f to get state of a job given in input (job_state)
 
 Returns : a string with the state (R, H, E...)
...
sub JOB_get_state{
	...
	if ( -e $qstat_out and !(-z $qstat_out) ){	
		...
		while (my $line = <QSTAT_OUT>){
			...
			#If the depend was found, check if continues on a newline
			if ($start == 1){
				###DON't KOW IF job_state could be more than one line...
			}
			#Find the line with depend = 
			if( $line =~ /job_state =/ ){
				#if ( defined $log_file){print_and_log( "depends are found...",$log_file);}	#DEBUGCODE
...
=head2 run_FREEBAYES_vc_parallel

 Title   : run_FREEBAYES_vc_parallel
 Usage   : run_FREEBAYES_vc_parallel(   );
...
sub run_FREEBAYES_vc_parallel{
	...
	my $target_bed = $cfg_hash->{'target_reg_f'}."/".get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_targetbed'},
					$cfg_hash->{'db_analysis_id'},$analysis_id);
	if (file_not_present($target_bed) > 0 ){ print_and_log("WARNING no target bed file is being used in freebayes!\n",$log_file);}
	...
	foreach my $bam_file ( @bam_files){
		if (file_not_present($bam_file) > 0 ){ die "Cannot proceed with $prog_used! Check: $bam_file.\n";}
	...
	my $numExec = 1;
		...
		#Settings to be launched in parallel using the splitted intervals
		...
		foreach my $exec (@chroms){
			...
			my $l_string = "";
			#Per intervals splitted with GATK splitIntervals
			##This code should be modified better...
			my $inter_num = $exec-1;
			my $suffix="";
			if ($inter_num<10){
				$suffix="000$inter_num";
			}else{
				$suffix="00$inter_num";
			}
			$l_string = " --targets ".$cfg_hash->{'target_intervals_fold'}."/$suffix-scattered.interval_list.bed ";

			
			my $ith_out = $outFolder."/".extract_name($out_file,1)."_$exec.".$cfg_hash->{'vcf_ext'};
			$out_files_str .= $ith_out.",";	
			
			#print_and_log( $cfg_hash->{'freebayes_path'}." - freebayes params: $param_str\n",$log_file);
			my $command = $cfg_hash->{'freebayes_path'}." -f ".$cfg_hash->{'hum_ref'}." $param_str $bam_inputs $l_string  > $ith_out";
			print_and_log("Executing command: $command \n",$log_file);
		...
		my $jobs_to_wait_dbi = "no_depend";
		if ($get_vcf ne "GET_VCF_LIST"){
			my $jobname = $cfg_hash->{'freebayes_vc_step_s'}."_a".$analysis_id;
			#Run a different job for each command
			$jobs_to_wait = execute_jobs($cfg_hash,$analysis_id,\@commands,$exec_job_script,$jobs_to_wait_dbi,$task,$jobname,"NO",$log_file);
		 }
		 
		chop($out_files_str);
		#Return the job ids for each job and the list of output file and the final output name
		return $jobs_to_wait, $out_files_str;
	#}else{					
		##Execute the command
		#my $command = $cfg_hash->{'freebayes_path'}." -f ".$cfg_hash->{'hum_ref'}." $param_str $bam_inputs  > $out_file";
		#print_and_log( "Executing command: $command\n",$log_file);
		#try_exec_command($command) or die "Unable to execute command: $command\n";		
	#}
}
...
sub try_exec_job{
	...
	if (defined $cfg_hash->{$suff.'_select'}){
		$qsub_resources .= "select=".$cfg_hash->{$suff.'_select'}.$cfg_hash->{'qsub_resource_sep'};
	}
	if (defined $cfg_hash->{$suff.'_ncpus'}){
		$qsub_resources .= "ncpus=".$cfg_hash->{$suff.'_ncpus'}.$cfg_hash->{'qsub_resource_sep'};
	}
	if (defined $cfg_hash->{$suff.'_nodes'}){
		$qsub_resources .= "nodes=".$cfg_hash->{$suff.'_nodes'}.$cfg_hash->{'qsub_resource_sep'};
	}
	if (defined $cfg_hash->{$suff.'_mem'}){
		$qsub_resources .= "mem=".$cfg_hash->{$suff.'_mem'}.$cfg_hash->{'qsub_resource_sep'};
	}
	chop($qsub_resources);	
...
=head2 execute_jobs
 ...
 Function: for each command it will launch a thread performing that operation
			...
			#dependency: list of jobids comma separated or "no_depend" otherwise
			
			#parallel_wait is used to allow that each command waits for a specific job.
				both commands and dependency are separated and each job waits for the same indexed jobid
...
sub execute_jobs{
  ...
  my $fold_suff = shift;
  my $name = shift;
  my $parallel_wait = shift;
  my $log_file = shift;
  
  my $jobs_num = scalar(@$commands);
  print_and_log("Executing $jobs_num jobs \n",$log_file);# I'll write a log in $log.\n"; 
	#print_and_log("foldsuff = $fold_suff name:$name \n",$log_file);#DEBUGCODE
	my $analysis_name = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    $cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_name'},
						    $cfg_hash->{'db_analysis_id'},$analysis_id);
	my $job_ids = "";
	...
	if ( $parallel_wait eq 'PAR_WAIT'){
		 #The number of commands should be equal to the number of jobs to wait
		 @dependencies = split(":",$dependency);
		 my $deps_num = scalar(@dependencies);
		 if ( $deps_num ne $jobs_num){
			log_and_exit("\nERROR (in execute_jobs) [jobname:$name]: The number of commands $jobs_num is different from the number of jobs to wait ( $deps_num)\n",$log_file);			 
		}
	}
	...
	foreach my $cmd (@$commands){
		...
		my $job_name = "$name\_$num_cmd";
		print_and_log("\nLaunching a job $job_name : ",$log_file);
		my $job_log = $cfg_hash->{$analysis_id.'_'.$fold_suff.'_log_fold'}."/$analysis_name\_$name\_$num_cmd.log";
		my $job_err = $cfg_hash->{$analysis_id.'_'.$fold_suff.'_log_fold'}."/$analysis_name\_$name\_$num_cmd.err";
		print_and_log("\nLog files: $job_log $job_err\n",$log_file);
		...
		#Check if there is a parallel wait and get the correspoding job to wait (-1 because the arrays start from 0)
		if ( $parallel_wait eq 'PAR_WAIT'){
			$dependency = $dependencies[$num_cmd-1];
		}
		#Call the program using qsub and giving it parameters for the job and
		my $env_vars = "COMMAND='$cmd' ";
		my $job_id = try_exec_job( $cfg_hash,$env_vars,$fold_suff,$program_call,$job_name,$dependency,$job_log,$job_err,$log_file);	
		$num_cmd++;
		$job_ids .= $job_id.":";
		print_and_log("Jobid:$job_id\n",$log_file);
...
sub check_config_parameters {
  ...
  if ( !($cfg_hash->{'qsub_walltime'} =~ /^\d+\:\d+:\d+$/) ){
	...
	my @tasks = ('qsub','qc','trim','align','refine','MS','varcall','genot','varfilt','phasing','stats',
			'anstats','finalout','fastfout','jobclean','VI','VCFB','VFFB','VCSAM','cnv','cnvo','XDOC');
	my @qsub_params = ("nodes","select","ncpus","threads");
  foreach my $task ( @tasks){
		if (defined $cfg_hash->{$task.'_mem'}){
			if ( !($cfg_hash->{$task.'_mem'} =~ /^\d+GB$/) ){
						log_and_exit("ERROR : ".$cfg_hash->{'qsub_mem'}." $task\_mem' must be set in this form NumberGB (e.g. 60GB). Please check it in config file \n",$log_file);
			}  
			#Remove GB and check that is lower than the maximum
			my $max = $cfg_hash->{'qsub_mem'};
			my $mem = $cfg_hash->{$task.'_mem'};
			$mem =~ s/GB//;
			$max =~ s/GB//;
			if ($mem > $max){
				$cfg_hash->{$task.'_mem'} = $max."GB";
			}
		}
		#qsub parameters that are positive integers
		foreach my $qsub_param (@qsub_params){
			if (defined $cfg_hash->{$task.'_'.$qsub_param }){
				if ( $cfg_hash->{$task.'_'.$qsub_param } ne ' ') {
					...
					if (defined $cfg_hash->{'qsub_'.$qsub_param} and length ($cfg_hash->{'qsub_'.$qsub_param}) > 0 and
						$cfg_hash->{'qsub_'.$qsub_param} ne ' ' ) {
						my $max = $cfg_hash->{'qsub_'.$qsub_param};
						my $val = $cfg_hash->{$task.'_'.$qsub_param};
						if ($val > $max){
							print_and_log("WARNING  $task\_$qsub_param value (".$cfg_hash->{$task.'_'.$qsub_param }.") is greater than the maximum for your system. Reducing it to $max... Please check it in config file \n",$log_file);							
							$cfg_hash->{$task.'_'.$qsub_param} = $max;
						}						
					}
